...
const HelpModal = ({ setIsOpen }: { setIsOpen: () => void }) => {
  ...
  useEffect(() => {
    const pageValues =
      pageToTexts.find((p) =>
        location.pathname.toLowerCase().includes(p.pageName.toLowerCase())
      ) ||
      pageToTexts[0];
    if (!pageValues) return;
    setTimeout(() => {
      setCurrentHeader(pageValues.header || "");
      setCurrentText(pageValues.text || "");
    }, 0);
  }, []);
  ...
  return (
    <>
      <div onClick={setIsOpen}>
        ...
        >
          <div className="absolute bottom-0 right-0 w-[200px] aspect-square animate-slide-out" id="fox">
            <img
